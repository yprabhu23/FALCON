...
class MotionLibBase():
    def __init__(self, motion_lib_cfg, num_envs, device):
        ...
        if "standardize_motion_length" in self.m_cfg.keys():
            ...
            self.standardize_motion_length_value = self.m_cfg.get("standardize_motion_length_value", 10)
        if flags.real_traj:
    ...
    def get_motion_state(self, motion_ids, motion_times, offset=None):
        ...
        if self.m_cfg.get("reverse_motion", False):
            ...
            motion_times[reverse_env_idx] = 2*motion_len[reverse_env_idx] - motion_times[reverse_env_idx]
    ...
    def load_motions(self, 
        ...
        if random_sample:
            if not self.uniform_sample:
                ...
                repeat_times = (self.num_envs + self._num_unique_motions - 1) // self._num_unique_motions  # 向上取整
                expanded_motion_idxes = motion_idxes.repeat(repeat_times)[:self.num_envs]
                sample_idxes = expanded_motion_idxes[torch.randperm(len(expanded_motion_idxes))[:self.num_envs]]
        else:
